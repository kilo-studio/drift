...
- [x] Card style as a `ViewModifier` (`.driftCard()`) — uniform 20pt padding, single `.glassEffect(.regular.tint(.driftSkyLowerMid.opacity(0.4)), in: RoundedRectangle(...))` for backdrop blur + sky-color tint. iOS 26 Liquid Glass composes blur and tint into one render pass. Earlier `.ultraThinMaterial` stacks (material + cream-tint fill + stroke + two near-zero shadows = 5 passes per card) were stripped after Instruments measured 170–370 offscreen passes per frame on Home and made scrolling unusably hitchy.
